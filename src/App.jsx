...
const defaultSettings = () => ({ bodyWeight: "", height: "", age: "", sex: "male", tdee: "", proteinGoal: "", carbsGoal: "", fatGoal: "", goalWeight: "", goalWeeks: "", stepsGoal: "10000", waterGoal: "8", weekPlan: { 1: "", 2: "", 3: "", 4: "", 5: "", 6: "", 0: "" } });
...
function SettingsModal({ settings, onSave, onClose }) {
  ...
  const calcMacros = () => {
    ...
    const height = parseRange(local.height);
    const age = parseRange(local.age);
    if (!bw) { setMacroMsg("Ingresá tu peso."); return; }
    if (!height) { setMacroMsg("Ingresá tu altura."); return; }
    if (!age) { setMacroMsg("Ingresá tu edad."); return; }
    // Mifflin-St Jeor
    const bmr = local.sex === "female"
      ? 10 * bw + 6.25 * height - 5 * age - 161
      : 10 * bw + 6.25 * height - 5 * age + 5;
    // Factor actividad moderada (gym 4-5x/semana)
    const tdeeCalc = Math.round(bmr * 1.55);
    // Déficit 20% para bajar grasa
    const tdeeDeficit = Math.round(tdeeCalc * 0.80);
    const prot = Math.round(bw * 1.8);
    const fatCal = Math.round(tdeeDeficit * 0.25);
    const fatG = Math.round(fatCal / 9);
    const protCal = prot * 4;
    const carbsCal = tdeeDeficit - protCal - fatCal;
    const carbsG = Math.max(Math.round(carbsCal / 4), 0);
    setLocal(s => ({ ...s, tdee: String(tdeeCalc), proteinGoal: String(prot), carbsGoal: String(carbsG), fatGoal: String(fatG) }));
    setMacroMsg(`✓ TDEE: ${tdeeCalc} kcal · Déficit 20%: ${tdeeDeficit} kcal · ${prot}g prot · ${carbsG}g carbos · ${fatG}g grasas`);
  };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}>
      <div style={{ background: "#13131f", border: "1px solid #2a2a3e", borderRadius: 16, padding: 24, width: "100%", maxWidth: 400, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <Lbl>Altura (cm)</Lbl>
        <input type="text" value={local.height || ""} onChange={e => setLocal(s => ({ ...s, height: e.target.value }))} placeholder="Ej: 178" style={{ ...inp, marginBottom: 14 }} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
          <div>
            <Lbl>Edad</Lbl>
            <input type="text" value={local.age || ""} onChange={e => setLocal(s => ({ ...s, age: e.target.value }))} placeholder="Ej: 25" style={{ ...inp, marginBottom: 0 }} />
          </div>
          <div>
            <Lbl>Sexo</Lbl>
            <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
              <button onClick={() => setLocal(s => ({ ...s, sex: "male" }))} style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: "none", cursor: "pointer", background: (local.sex || "male") === "male" ? "#7c3aed" : "#1a1a2e", color: (local.sex || "male") === "male" ? "#fff" : "#666", fontSize: 12, fontWeight: 600 }}>♂ Hombre</button>
              <button onClick={() => setLocal(s => ({ ...s, sex: "female" }))} style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: "none", cursor: "pointer", background: local.sex === "female" ? "#7c3aed" : "#1a1a2e", color: local.sex === "female" ? "#fff" : "#666", fontSize: 12, fontWeight: 600 }}>♀ Mujer</button>
            </div>
          </div>
        </div>
        <Lbl>TDEE (kcal de mantenimiento)</Lbl>
        <input type="text" value={local.tdee} onChange={e => setLocal(s => ({ ...s, tdee: e.target.value }))} placeholder="Se calcula automáticamente →" style={{ ...inp, marginBottom: 14 }} />
        ...
        <div style={{ background: "#0d0d1a", border: "1px solid #7c3aed33", borderRadius: 10, padding: "12px 14px", marginBottom: 14 }}>
          ...
          <button onClick={calcMacros} style={{ width: "100%", padding: "8px 0", borderRadius: 8, border: "none", background: "#7c3aed22", color: "#7c3aed", cursor: "pointer", fontSize: 12, fontWeight: 600, marginBottom: 12 }}>
            ✨ Calcular TDEE y macros automáticamente
          </button>
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a14", fontFamily: "'Inter', system-ui, sans-serif", color: "#e2e8f0", padding: "24px 14px 0" }}>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ fontSize: 26, fontWeight: 900, letterSpacing: -1 }}>AGON</div>
        </div>
