...
const GEMINI_API_KEY = "AQ.Ab8RN6LDKlnkR3GN00YfXd8CjlateQI8zMsbQXsU3rf_O35y7g";
...
async function callClaude(body) {
  // Extrae el texto del mensaje (soporta string o array de content blocks)
...
function AddMealModal({ onAdd, onClose, recentMeals = [] }) {
  ...
  const handleContinue = async () => {
    ...
    setChecking(true);
    try {
      const result = await checkMealAmbiguity(t);
      if (result.necesita_aclaracion && result.preguntas?.length > 0) {
        setQuestions(result.preguntas);
        setAnswers(result.preguntas.map(() => ""));
        setStep("clarify");
      } else {
...
function DayView({ date, data, onChange, settings, allData, routines, showDashboard = false }) {
  ...
  const addMeal = async ({ name, mealTime, aclaraciones = "", photoBase64 = null }) => {
    const id = Date.now();
    const newMeal = { id, name, mealTime, loading: true, nutrition: null, error: false };
    ...
    try {
      ...
      const updated = mealsWithNew.map(m => m.id === id ? { ...m, loading: false, error: true } : m);
      onChange({ ...data, meals: updated });
    }
  };
